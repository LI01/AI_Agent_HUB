# Phase 2.3 Design v3 — LLM-CLI Worker Skills + Per-CLI Adapters

**Author:** designer-v3 (sub-agent)
**Date:** 2026-05-09
**Scope:** Same as v2. Three skills (`agent-spawn`, `agent-tasks`, `agent-close`) + three per-CLI adapters (`codex`, `claude_code`, `opencode`) + one shared role-preset module + small shared helpers. Additive only. NO new top-level deps.

---

## 0. Diff vs v2

Codex review v2 returned REVISE with 2 outstanding items. Both applied verbatim; nothing else changed. The 12 previously-resolved findings (and the 3 accept items) stay verbatim from v2.

| # | Where | What changed vs v2 |
|---|---|---|
| 3 | §3.3, §8 | Codex parser rewritten to handle BOTH startswith-marker AND embedded-after-newline cases. Old parser only checked `\n[assistant]:` / `\nassistant:` after `strip()`, which incorrectly failed the fixture `"\nassistant: hello world" -> "hello world"` (the leading newline was stripped). New parser tries `startswith(marker)` first, then `"\n" + marker` rsplit. §8 fixture list unchanged — all four cases now pass. |
| 13 | §2.3, §5 | Close-skill PID-check flow rewritten so `pid_is_adapter() == None` (inconclusive) is distinguished from `False` (alive but reused) and from `not pid_alive` (dead). Old `pid_reused = local and not pid_is_adapter(...)` returned truthy for `None` (because `not None == True`), incorrectly treating inconclusive as PID reuse. New flow defines `pid_check_inconclusive`, `pid_reused`, `terminated` separately; only `pid_check is True` triggers SIGTERM/SIGKILL; purge guard now also checks `not pid_check_inconclusive`. The `test_p23_llm_6b_close_skill_pid_check_inconclusive` case is now implementable. |

All other sections — §0a Diff vs v1 (kept for traceability), §1, §2.0–§2.2, §3.0–§3.2, §3.4–§3.5, §4, §6, §7, §9, §10 — are byte-for-byte identical to v2 (unchanged from v2).

---

## 0a. Diff vs v1

(unchanged from v2 — see `design/phase2.3/design_v2.md` §0 for the full 14-row table mapping each v1→v2 fix to its section. All 14 v1 revise items resolved in v2; codex-`accept` items kept as-is, with the v1 finding #15 rename: opencode session marker is `.opencode-session-started`.)
...
**Kill sequence (per selected agent) — verbatim per review-v2 finding #13:**
...
This correctly distinguishes the four states: dead (`not pid_alive` → `terminated`), alive-and-adapter (`pid_check is True` → kill), alive-but-reused (`pid_check is False` → skip kill, set `pid_reused`), alive-and-inconclusive (`pid_check is None` → skip kill, set `pid_check_inconclusive`). Crucially, `not None` is no longer used as a boolean — `pid_check is False` and `pid_check is None` are tested explicitly.

**`--purge` ordering (verbatim per review-v2 finding #13):**

```python
if purge and terminated and not pid_reused and not pid_check_inconclusive and workdir_under_agent_hub_pipelines(workdir):
...
The added `not pid_check_inconclusive` clause prevents purge when we cannot confirm the PID is no longer an adapter. Combined with the kill-sequence change above, an inconclusive PID check now correctly skips both kill AND purge while still calling `/unregister`.
...
def _parse_codex_output(stdout: str) -> str:
    """Per codex review-v2 finding #3: handle BOTH startswith-marker AND
    embedded-after-newline cases. Strip first, then try each marker as a
    prefix; failing that, look for the same marker after a newline and
    rsplit on the last occurrence. Falls back to the full stripped stdout
...
Parser fixture tests in §8 cover four cases: embedded `\n[assistant]:` marker, leading-newline `assistant:` marker (now handled via `startswith` after `strip()`), no marker (full stdout returned), and empty stdout (returns `""`). All four pass with the replacement above.
...
| `test_p23_llm_6b_close_skill_pid_check_inconclusive` | FR-LLM-6b | Mock `pid_is_adapter` → `None` and `pid_is_alive` → `True`. Call `close.py --ids <id> --purge` with `workdir` under `~/.agent-hub/pipelines/`. Verify: NO `os.kill` is called; `/unregister` IS called; per-row report contains `pid_check_inconclusive: True`; report does NOT contain `pid_reused`; report does NOT incorrectly mark `terminated`; `shutil.rmtree` is NOT called (purge skipped); report contains `purge_skipped_process_still_alive_or_untrusted_path`. Implementable now that the §2.3 flow distinguishes `pid_check is None` (inconclusive) from `pid_check is False` (reused). |
...
**End of design v3.** Codex review-v2's 2 outstanding findings (#3 codex parser, #13 close-skill PID-check flow) applied verbatim; the other 12 v1 revise findings and 3 accept items remain unchanged from v2; no new top-level deps; full backward compatibility with Phase 1/1.5/2/2.2; ≥ 6 pytest cases (16 total here, with `test_p23_llm_6b` now implementable).
