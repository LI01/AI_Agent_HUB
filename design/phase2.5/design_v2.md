# Phase 2.5 Design v2 — MCP Spec Compliance + Cross-Client Verification

**Status:** designer-v2, incorporates reviewer-v1 findings verbatim.
...
Plus **compatibility-CLI regression tests** for the three updated `mcp/server.py` functions, also owned by Slice C, exercised against the running TestClient via the `HttpHubBackend`-style code path:

- `test_p25_mcp_compat_list_agents_returns_list`: import `mcp.server`, point it at the test hub, call `mcp.server.list_agents()` → assert result is a `list` (not a dict), and that registering an agent first then calling it returns a 1-element list containing that agent's id.
- `test_p25_mcp_compat_list_tasks_returns_list`: parallel; `mcp.server.list_tasks()` returns a `list`.
- `test_p25_mcp_compat_list_api_keys_returns_list`: parallel; `mcp.server.list_api_keys()` returns a `list`.
